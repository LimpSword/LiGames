...
public class ItemBuilder {
    ...
    private boolean leatherArmor = false;
    private Color color;
    private String key;
    ...
    public ItemBuilder(ItemStack item) {
        ...
        if (item.hasItemMeta())
            this.lore = item.getItemMeta().getLore() == null ? new ArrayList<>() : item.getItemMeta().getLore();
        if (item.hasItemMeta())
    ...
    public static ItemStack setHead(String name, String url, List<String> lores, boolean glowing) {
        ItemStack head = new ItemStack(Material.PLAYER_HEAD, 1);
        SkullMeta headMeta = (SkullMeta) head.getItemMeta();
        GameProfile profile = new GameProfile(UUID.randomUUID(), "Hello World");
        headMeta.setDisplayName(name);
        if (lores != null) headMeta.setLore(lores);
        if (glowing) headMeta.addEnchant(Enchantment.DURABILITY, 0, false);
    ...
    public ItemBuilder leatherArmor(boolean leatherArmor) {
    ...
    /**
     * Sets the model data of the ItemStack
     *
     * @param modelData Model data for the ItemStack
     */
    ...
    public ItemBuilder persistentString(String key, String persistentString) {
        this.key = key;
    ...
    public ItemStack build() {
        ...
        /*if (key != null && persistentString != null) {
            meta.getPersistentDataContainer().set(namespacedKey == null ? new NamespacedKey(IElyAPI.get().bukkit().plugin(), key) : namespacedKey, PersistentDataType.STRING, persistentString);
        }*/

        if (leatherArmor && color != null) {
